            ...
            <section id='remove'  style='display:none'>
             ...
             <div class="form-group">
              ...
              <script type="text/javascript">
                ...
                <?php echo $this->Form->hidden('User.is_confirm', array('value'=> $is_confirm, 'id'=>'user-info-id'))?>
              <?php if($user['User']['status_id'] == 2){?>
              <div class="form-group">
                <div class="col-lg-10 col-lg-offset-2">
                  <button type="submit" class="btn btn-primary" id="btn-confirm-user-info" >Confirm</button>
                  <button type="button" class="btn btn-primary" id="btn-save-user-info" >Save</button>
              ...
              <?php }?>
                ...
                <script type="text/javascript" >
                <?php if($is_confirm == "1"){?>
                  $('#btn-confirm-user-info').hide();
                  $('#btn-save-user-info').show();
                  $('#btn-cancel-user-info').show();
                  $('#UserEditBasicInfo').find(':button:not(#btn-save-user-info):not(#btn-cancel-user-info)').hide();
                  $('#UserEditBasicInfo').find(':input:not(:disabled):not(:hidden):not(#btn-save-user-info):not(#btn-cancel-user-info)').prop('disabled',true);
                <?php }else {?>
                   $('#btn-confirm-user-info').show();
                ...
                <?php } ?>    
                   ...
                   $('#btn-cancel-user-info').on('click', function() {
                      $('#UserEditBasicInfo').find(':input:not(#btn-save-user-info):not(#btn-cancel-user-info)').prop('disabled',false);
                      $('#btn-cancel-user-info').hide();
                      $('#btn-save-user-info').hide();
                      $('#btn-confirm-user-info').show();
                      $('#UserEditBasicInfo').find(':button:not(#btn-save-user-info):not(#btn-cancel-user-info)').show();
                      $('#user-info-id').val("");

                   });
                  $('#btn-save-user-info').on('click', function(){
                      $.ajax({
                        url: "<?php echo $this->webroot?>users/update_basic_info", 
                        'method':'POST',
                        'data':"data[User][is_confirm]=1",
